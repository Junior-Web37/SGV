...
public interface PurchaseRepository extends JpaRepository<Purchase, Long> {
    ...
    @Query("SELECT p FROM Purchase p WHERE p.supplier.id = :supplierId AND p.state <> 'CANCELLED' AND (COALESCE(p.total, 0.0) - COALESCE(p.paidAmount, 0.0)) > 0.01 ORDER BY p.createdAt DESC")
     ...
     */
    @Query("SELECT p.supplier.id, COALESCE(SUM(p.total), 0.0) - COALESCE(SUM(p.paidAmount), 0.0) " +
