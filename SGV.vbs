...
Dim jarPath, workDir
If FSO.FileExists(strScriptDir & "\java-sgv\target\java-sgv-0.1.0.jar") Then
    jarPath = strScriptDir & "\java-sgv\target\java-sgv-0.1.0.jar"
    workDir = strScriptDir & "\java-sgv"
ElseIf FSO.FileExists(strScriptDir & "\target\java-sgv-0.1.0.jar") Then
    jarPath = strScriptDir & "\target\java-sgv-0.1.0.jar"
    workDir = strScriptDir
Else
    ' Se o JAR ainda nao foi gerado, corre o SGV-Launcher.bat
    If FSO.FileExists(strScriptDir & "\SGV-Launcher.bat") Then
        WshShell.CurrentDirectory = strScriptDir
        WshShell.Run """" & strScriptDir & "\SGV-Launcher.bat""", 1, False
        WScript.Quit 0
    ElseIf FSO.FileExists(strScriptDir & "\java-sgv\SGV-Launcher.bat") Then
        WshShell.CurrentDirectory = strScriptDir & "\java-sgv"
        WshShell.Run """" & strScriptDir & "\java-sgv\SGV-Launcher.bat""", 1, False
        WScript.Quit 0
    End If
End If
