-- SQLite 3NF schema for SGV (Moçambique)
...
CREATE TABLE IF NOT EXISTS sales (
  ...
  FOREIGN KEY(origin_sale_id) REFERENCES sales(id)
);
...
CREATE TABLE IF NOT EXISTS movements_cash (
  ...
  note TEXT,
  created_at TEXT DEFAULT (datetime('now')),
  FOREIGN KEY(session_id) REFERENCES sessions_cash(id) ON DELETE CASCADE
);
...
CREATE TABLE IF NOT EXISTS expenses (
  ...
  paid_at TEXT,                         -- Data de pagamento
...
-- Unidades padrão (podem ser inseridas via seed)
INSERT OR IGNORE INTO metric_units (abbreviation, description) VALUES
  ('UN', 'Unidade'),
  ('KG', 'Quilograma'),
  ('G', 'Grama'),
  ('LT', 'Litro'),
